...
- 数据存储：文件系统 + SQLite
- LLM：小米 MiMo / MiniMax（多Provider支持）
...
```bash
git clone https://github.com/xxx/kioxus.git
cd kioxus
```
...
编辑 `config/config.yaml`，填入 LLM Provider 的 API Key。
...
- 四层记忆架构，BM25搜索，渐进压缩
- 支持小米MiMo和MiniMax两种LLM Provider
...
- 可扩展LLM Provider到 `src/core_v2/llm.py`
- 可对接前端页面实现可视化管理
- 可接入Docker一键部署
- 可新增定时任务、权限系统
...
项目经过完整功能测试、边界测试、稳定性测试，43个测试用例覆盖核心模块，核心功能运行稳定，可正常投入使用与二次开发。
...
| 2026-08-30 | v0.3.0 | 项目整理，文档统一 |

## 开源协议

本项目基于 MIT License 开源，可免费学习、使用和二次开发。
